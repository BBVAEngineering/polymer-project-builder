...
function transformScript(input) {
	const babelOptions = {
		plugins: [require.resolve('babel-plugin-transform-block-scope-to-iife')]
	};

	const { code } = babel.transformSync(input, babelOptions);
...
module.exports = class PolymerProjectBuilder {
	...
	splitHtmlAndJs() {
		if (this.buildConfig.csp) {
			...
			fs.writeFileSync(path.join(this.options.dest, jsFileName), transformScript(js));
		}
	}
